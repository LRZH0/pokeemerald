...
static void PrintMovePowerAndAccuracy(u16 moveIndex)
{
    ...
    if (moveIndex != 0)
    {
        ...
        if (moveIndex == MOVE_HIDDEN_POWER && gSaveBlock2Ptr->optionsMoveInfo == 0)
        {
        ...
        else if (moveIndex == MOVE_RETURN && gSaveBlock2Ptr->optionsMoveInfo == 0)
        {
            ConvertIntToDecimalStringN(gStringVar1, (10 * monFriendship / 25), STR_CONV_MODE_RIGHT_ALIGN, 3);
            text = gStringVar1;
        }
        else if (moveIndex == MOVE_FRUSTRATION && gSaveBlock2Ptr->optionsMoveInfo == 0)
        {
...
static void SetMoveTypeIcons(void)
{
    ...
    for (i = 0; i < MAX_MON_MOVES; i++)
    {
        if (summary->moves[i] != MOVE_NONE) {
            if (summary->moves[i] == MOVE_HIDDEN_POWER && gSaveBlock2Ptr->optionsMoveInfo == 0) {
...
static void SetNewMoveTypeIcon(void)
{
    ...
    {
        if (sMonSummaryScreen->currPageIndex == PSS_PAGE_BATTLE_MOVES)
            if (sMonSummaryScreen->newMove == MOVE_HIDDEN_POWER && gSaveBlock2Ptr->optionsMoveInfo == 0) {
                u8 typeBits  = ((GetMonData(mon, MON_DATA_HP_IV) & 1) << 0)
                     | ((GetMonData(mon, MON_DATA_ATK_IV) & 1) << 1)
                     | ((GetMonData(mon, MON_DATA_DEF_IV) & 1) << 2)
                     | ((GetMonData(mon, MON_DATA_SPEED_IV) & 1) << 3)
